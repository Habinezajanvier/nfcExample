...
class MifareClassic {
  ...
  /// Get block count in a sector
  ///
  /// This uses MifareClassic#getBlockCountInSector API on android

  Future<int> getBlockCountInSector({required int sectorIndex}) async {
    return channel.invokeMethod('MifareClassic#getBlockCountInSector', {
      'handle': _tag.handle,
      'sectorIndex': sectorIndex
    }).then((value) => value!);
  }

  /// Get block index from sector
  ///
  /// This uses MifareClassic#sectorToBlock API on Android

  Future<int> sectorToBlock({required int sectorIndex}) async {
    return channel.invokeMethod('MifareClassic#sectorToBlock', {
      'handle': _tag.handle,
      'sectorIndex': sectorIndex
    }).then((value) => value!);
  }
