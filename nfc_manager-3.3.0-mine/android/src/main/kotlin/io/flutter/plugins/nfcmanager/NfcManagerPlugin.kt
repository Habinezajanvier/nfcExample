...
class NfcManagerPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
  ...
  override fun onMethodCall(call: MethodCall, result: Result) {
    when (call.method) {
      ...
      "MifareClassic#authenticateSectorWithKeyA" ->
      ...
      "MifareClassic#sectorToBlock" -> handleMifareClassicSectorToBlock(call, result)
      "MifareClassic#getBlockCountInSector" ->
          handleMifareClassicGetBlockCountInSector(call, result)
  ...
  private fun handleNfcStartSession(call: MethodCall, result: Result) {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
      ...
      adapter.enableReaderMode(
          ...
          getFlags(call.argument<List<String>>("pollingOptions")!!),
          null
      )
      result.success(null)
    }
  ...
  private fun handleNfcDisposeTag(call: MethodCall, result: Result) {
    val tag =
        tags.remove(call.argument<String>("handle")!!)
            ?: run {
              result.success(null)
              return
            }
  ...
  private fun handleMifareClassicGetBlockCountInSector(call: MethodCall, result: Result) {
    tagHandler(call, result, { MifareClassic.get(it) }) {
      val sectorIndex = call.argument<Int>("sectorIndex")!!
      result.success(it.getBlockCountInSector(sectorIndex))
    }
  }

  private fun handleMifareClassicSectorToBlock(call: MethodCall, result: Result) {
    tagHandler(call, result, { MifareClassic.get(it) }) {
      val sectorIndex = call.argument<Int>("sectorIndex")!!
      result.success(it.sectorToBlock(sectorIndex))
    }
  }
  ...
  private fun <T : TagTechnology> tagHandler(
    ...
    val tag =
        tags[call.argument<String>("handle")!!]
            ?: run {
